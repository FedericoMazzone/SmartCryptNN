...
class AttackData:
    def __init__(
        ...
        logging.info(f"Dataset size: {len(m_features)}m-{len(nm_features)}nm")
        ...
        logging.info(
            f"Attack training set size: {len(mtrain)}m-{len(nmtrain)}nm")
        logging.info(
            f"Attack test     set size: {len(mtest)}m-{len(nmtest)}nm")
